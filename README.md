...
1. Make sure you're logged into npm with access to the `@nast` organization:

```bash
npm login
npm access ls-packages  # Verify you have access to @nast
```
...
Update version in individual packages:
...
Or use a version management tool like `changeset`:
